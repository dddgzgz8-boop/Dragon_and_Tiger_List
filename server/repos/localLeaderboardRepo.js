...
export async function writePointsSync(sync) {
  ...
  cycle.point = sync.totals.map(item => { let pid = peopleByName.get(item.name);if (!pid) { pid = `feishu-${item.name.toLowerCase().replace(/\s+/g, '-')}`;cycle.people[pid] = { name: item.name, store: '待设置', level: '待设置' } }const feishuPerson=sync.people?.[item.name]||{};cycle.people[pid].feishuOpenId=feishuPerson.openId||cycle.people[pid].feishuOpenId||'';cycle.people[pid].avatarUrl=feishuPerson.avatarUrl||cycle.people[pid].avatarUrl||'';if(feishuPerson.level&&(!cycle.people[pid].level||cycle.people[pid].level==='待设置'))cycle.people[pid].level=feishuPerson.level;const detail=sync.details[item.name]||[];pointDetails[pid]=detail;const goldPoints=detail.filter(x=>/黄金/.test(x.cardType)).reduce((sum,x)=>sum+Number(x.quantity||0),0),ordinaryPoints=detail.filter(x=>!/黄金/.test(x.cardType)).reduce((sum,x)=>sum+Number(x.quantity||0),0);return { pid, points: item.points, ordinaryPoints, goldPoints, change: 0 } }).sort((a,b)=>b.points-a.points).map((row,index)=>({...row,rank:index+1}))
